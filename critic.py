...
class DdpgCriticNetwork(tf.keras.Model):
    def __init__(self, state_dim, action_dim, hidden_dim):
        ...
        self.inputs = tf.keras.layers.InputLayer(input_shape=(action_dim, 1700))
...
class DdpgAveCriticNetwork(tf.keras.Model):
    def __init__(self, state_dim, action_dim, hidden_dim):
        super(DdpgAveCriticNetwork, self).__init__()
        self.inputs = tf.keras.layers.InputLayer(input_shape=(action_dim, 10*state_dim))
        self.fc1 = tf.keras.layers.Dense(action_dim, activation='relu')
        self.normal1 = tf.keras.layers.BatchNormalization\
            (axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, beta_initializer='zeros',
             gamma_initializer='ones', moving_mean_initializer='zeros',
...
class DdpgCritic(object):
    ...
    def __init__(self, hidden_dim, learning_rate, state_dim,action_dim, target_network_update_rate):
        ...
        # build并summary
        self.network([np.zeros((1, self.action_dim)),np.zeros((1, 1700))])
        self.target_network([np.zeros((1, self.action_dim)),np.zeros((1, 1700))])
    ...
    def get_state(self,user_id,all_history_dict,embedding_network):
        user_history = all_history_dict[user_id]
        user_history_eb = embedding_network.get_layer('movie_embedding')(user_history)
        for eb in user_history_eb:
            eb = self.network(eb)
        return eb
...
class DdpgAveCritic(object):
    ...
    def __init__(self, hidden_dim, learning_rate, state_dim,action_dim, target_network_update_rate):
        ...
        self.hidden_dim = hidden_dim
        # critic network / target network
        self.network = DdpgAveCriticNetwork(state_dim=state_dim,action_dim=action_dim,hidden_dim=hidden_dim)
        self.target_network = DdpgAveCriticNetwork(state_dim=state_dim,action_dim=action_dim,hidden_dim=hidden_dim)
        # build并summary
        self.network([np.zeros((1, self.action_dim)),np.zeros((1, state_dim*10))])
        self.target_network([np.zeros((1, self.action_dim)),np.zeros((1, state_dim*10))])
        print("ddpg_critic_network已创建")
        print()
        self.network.summary()
        print("ddpg_target_critic_network已创建")
